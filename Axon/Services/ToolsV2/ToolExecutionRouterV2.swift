...
final class ToolExecutionRouterV2: ObservableObject {
    ...
    // MARK: - Provider Handler Resolution

    private func resolveProviderHandlerId(_ provider: String) -> String {
    ...
    private func executeProviderNativeV2(
        ...
        let handlerId = resolveProviderHandlerId(provider)
        guard let handler = handlerRegistry.handlerV2(for: handlerId) else {
        ...
        logger.info("Routing provider_native tool '\(loadedTool.id)' to handler '\(handlerId)'")
