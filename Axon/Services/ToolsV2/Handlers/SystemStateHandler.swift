...
final class SystemStateHandler: ToolHandlerV2 {
    ...
    private func changeModel(provider providerString: String, model modelId: String, reasoning: String) async -> ToolResultV2 {
        ...
        // 2. Validate model exists for this provider (check registry first, fall back to enum)
        let registryModels = UnifiedModelRegistry.shared.chatModels(for: targetProvider)
        let availableModels = registryModels.isEmpty ? targetProvider.availableModels : registryModels
    ...
    private func changeProvider(provider providerString: String, reasoning: String) async -> ToolResultV2 {
        ...
        // 4. Apply the change (use first available model for the new provider from registry)
        let providerModels = UnifiedModelRegistry.shared.chatModels(for: targetProvider)
        let defaultModel = providerModels.first?.id ?? targetProvider.availableModels.first?.id ?? ""
        let defaultModelName = providerModels.first?.name ?? targetProvider.availableModels.first?.name ?? defaultModel
