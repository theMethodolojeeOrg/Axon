...
final class ConversationTitleOrchestrator {
    ...
    private func generateTitleWithSelectedProvider(
        ...
        let effectiveModel: String = {
            ...
            // default model to avoid mismatched model IDs.
            if let forced = fallbackProviderStringIfAppleSelected {
                switch forced {
                case "openai":
                    // Prefer a small/cheap default if present.
                    return AIProvider.openai.availableModels.first(where: { $0.id.contains("mini") })?.id
                        ?? AIProvider.openai.availableModels.first?.id
                        ?? "gpt-4o-mini"
                case "anthropic":
                    return AIProvider.anthropic.availableModels.last?.id
                        ?? AIProvider.anthropic.availableModels.first?.id
                        ?? "claude-haiku-4-5-20251001"
                case "gemini":
                    return AIProvider.gemini.availableModels.first?.id
                        ?? "gemini-2.5-flash"
                default:
                    return model
                }
