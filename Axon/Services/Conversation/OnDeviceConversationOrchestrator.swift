...
class OnDeviceConversationOrchestrator: ConversationOrchestrator {
    ...
    func sendMessage(
        ...
        let multiFileArtifactsEnabled = await MainActor.run {
            SettingsViewModel.shared.settings.toolSettings.multiFileArtifactsEnabled
        }
        let basePrompt = buildAgentBasePrompt(
            hasMemoryTool: enabledTools.contains(ToolId.createMemory.rawValue),
            userName: userName,
            multiFileArtifactsEnabled: multiFileArtifactsEnabled
        )
    ...
    private func buildAgentBasePrompt(
        hasMemoryTool: Bool,
        userName: String?,
        multiFileArtifactsEnabled: Bool
    ) -> String {
        ...
        if multiFileArtifactsEnabled {
            prompt += """


            Multi-file artifact output:
            - When generating a multi-file web artifact, I emit each file in its own fenced code block using: `language:relative/path.ext`.
            - I keep paths relative only (no leading `/` and no `..` path traversal).
            - I include an HTML entry file (prefer `index.html`) and put CSS/JS in separate files.
            - I keep tool calls in `tool_request` fenced blocks exactly as required; I do not path-tag `tool_request`.
            """
        }
    ...
    /// Resolve MIME type for attachment using shared MIME normalization policy.
    ...
    private func callAppleFoundation(system: String?, messages: [Message]) async throws -> String {
        try await OnDeviceTextGenerator.appleFoundation(system: system, messages: messages)
    }
    ...
    private func callLocalMLX(
        ...
        try await OnDeviceTextGenerator.localMLX(
            modelId: modelId,
            system: system,
            messages: messages,
            modelParams: modelParams
