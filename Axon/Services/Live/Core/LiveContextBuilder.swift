...
class LiveContextBuilder {
    ...
    private func buildLiveBasePrompt() -> String {
        ...
        if SettingsStorage.shared.loadSettingsOrDefault().toolSettings.multiFileArtifactsEnabled {
            prompt += """


            Multi-file artifact output:
            - When generating a multi-file web artifact, emit each file as a fenced code block with `language:relative/path.ext`.
            - Keep paths relative only (no leading `/` and no `..` path traversal).
            - Prefer `index.html` as the entry file and separate CSS/JS into their own files.
            """
        }
