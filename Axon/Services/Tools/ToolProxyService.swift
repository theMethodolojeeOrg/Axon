...
class ToolProxyService: NSObject, ObservableObject, CLLocationManagerDelegate {
    ...
    private func executeQuerySystemState(query: String) async -> ToolResult {
        ...
        case "providers":
            ...
            for provider in AIProvider.allCases {
                ...
                // Prefer registry models, fall back to enum
                let models = UnifiedModelRegistry.shared.chatModels(for: provider)
                let availableModels = models.isEmpty ? provider.availableModels : models
    ...
    private func executeChangeSystemState(query: String) async -> ToolResult {
        ...
        case "model", "provider":
            ...
            // Validate model exists for this provider (check registry first, fall back to enum)
            let registryModels = UnifiedModelRegistry.shared.chatModels(for: provider)
            let providerModels = registryModels.isEmpty ? provider.availableModels : registryModels
            let validModels = providerModels.map { $0.id }
                ...
                return ToolResult(
                    ...
                    result: """
                        ...
                        **Valid models for \(provider.displayName):**
                        \(providerModels.map { "- `\($0.id)` (\($0.name))" }.joined(separator: "\n"))
                        """,
                    sources: nil,
                    memoryOperation: nil
                )
