...
final class CreativeGalleryService: ObservableObject {
    ...
    func items(for filter: GalleryFilter) -> [CreativeItem] {
        ...
        case .views:
            return items.sorted { $0.displayTitle.count > $1.displayTitle.count }
        }
    }
    ...
    /// Create an editable fork from an immutable workspace snapshot and persist it.
    func createEditableForkWorkspace(from snapshot: ArtifactWorkspace) -> ArtifactWorkspace? {
        guard !snapshot.files.isEmpty else { return nil }
        if snapshot.isEditableFork && !snapshot.isReadOnlySnapshot {
            return snapshot
        }

        var fork = snapshot
        fork.id = "workspace_fork_\(UUID().uuidString)"
        fork.isEditableFork = true
        fork.isReadOnlySnapshot = false
        fork.sourceItemId = snapshot.sourceItemId ?? snapshot.messageId.map { "\($0)_bundle" }
        fork.createdAt = Date()
        fork.updatedAt = Date()

        saveWorkspaceFork(fork)
        return fork
    }

    /// Persist a workspace fork into CreativeItem storage and update in-memory state.
    func saveWorkspaceFork(_ workspace: ArtifactWorkspace) {
        guard let encoded = try? JSONEncoder().encode(workspace),
              let bundleJSON = String(data: encoded, encoding: .utf8) else {
            return
        }
        ...
        let item = CreativeItem(
            ...
            artifactEntryPath: workspace.entryPath,
            sourceItemId: workspace.sourceItemId,
            isEditableFork: workspace.isEditableFork
        )
    ...
    private func entityToCreativeItem(_ entity: NSManagedObject) -> CreativeItem? {
        ...
        return CreativeItem(
            ...
            artifactBundleJSON: entity.value(forKey: "artifactBundleJSON") as? String,
            artifactEntryPath: entity.value(forKey: "artifactEntryPath") as? String,
            sourceItemId: entity.value(forKey: "sourceItemId") as? String,
            isEditableFork: entity.value(forKey: "isEditableFork") as? Bool ?? false,
    ...
    private func creativeItemToEntity(_ item: CreativeItem, entity: NSManagedObject) {
        ...
        entity.setValue(item.artifactBundleJSON, forKey: "artifactBundleJSON")
        entity.setValue(item.artifactEntryPath, forKey: "artifactEntryPath")
        entity.setValue(item.sourceItemId, forKey: "sourceItemId")
        entity.setValue(item.isEditableFork, forKey: "isEditableFork")
        ...
        do {
            ...
            for message in messages {
                ...
                if let content = message.content, let conversationId = message.conversationId, let messageId = message.id {
                    ...
                    // Extract multi-file workspace artifacts from path-tagged fences.
                    let workspaceItems = extractWorkspaceArtifactsFromContent(
                        content,
                        conversationId: conversationId,
                        messageId: messageId,
                        timestamp: message.timestamp ?? Date()
    ...
    private func extractWorkspaceArtifactsFromContent(
        _ content: String,
        conversationId: String,
        messageId: String,
        timestamp: Date
    ) -> [CreativeItem] {
        let segments = FencedCodeParser.split(content)
        let workspaceId = "\(messageId)_bundle_workspace"
        let itemId = "\(messageId)_bundle"

        guard let workspace = ArtifactWorkspaceAssembler.workspace(
            from: segments,
    ...
    private func extractArtifactsFromContent(_ content: String, conversationId: String, messageId: String, timestamp: Date) -> [CreativeItem] {
        ...
        for (index, match) in matches.enumerated() {
            ...
            // Path-tagged fences (language:path/file.ext) are handled as workspace bundles.
            if let title {
