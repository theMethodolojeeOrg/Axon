...
struct CreativeItem: Identifiable, Codable, Equatable {
    ...
    let artifactBundleJSON: String? // Multi-file workspace payload (if artifact bundle)
    let artifactEntryPath: String?  // Preferred entry file inside bundle
    let sourceItemId: String?       // Original snapshot item when this is an editable fork
    let isEditableFork: Bool        // True when item is a mutable fork of a snapshot
    ...
    init(
        ...
        language: String? = nil,
        fileSize: Int64? = nil,
        artifactBundleJSON: String? = nil,
        artifactEntryPath: String? = nil,
        sourceItemId: String? = nil,
        isEditableFork: Bool = false,
        ...
        self.artifactBundleJSON = artifactBundleJSON
        self.artifactEntryPath = artifactEntryPath
        self.sourceItemId = sourceItemId
        self.isEditableFork = isEditableFork
    ...
    var fileExtension: String {
        ...
        case .artifact:
            if artifactBundleJSON != nil { return "json" }
            return language ?? "txt"
        }
    }

    var artifactWorkspace: ArtifactWorkspace? {
        guard type == .artifact,
              let artifactBundleJSON,
              let data = artifactBundleJSON.data(using: .utf8) else {
            return nil
        }
        return try? JSONDecoder().decode(ArtifactWorkspace.self, from: data)
    }
}
...
enum GalleryFilter: Equatable, Identifiable {
    ...
    case views  // Generative UI views
