...
struct LaunchScreenView: View {
    var body: some View {
        GeometryReader { geometry in
            LaunchScreenContent(screenSize: geometry.size)
                .ignoresSafeArea()
        }
    }
}

private struct LaunchScreenContent: View {
    ...
    init(screenSize: CGSize) {
        self._physicsEngine = StateObject(wrappedValue: BubblePhysicsEngine(
    ...
    var body: some View {
        ZStack {
            ...
            VStack {
                Spacer()
                Text("Axon by methodolojee, MIT 2025")
