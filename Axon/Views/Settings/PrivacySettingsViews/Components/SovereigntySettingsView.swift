    ...
    var body: some View {
        ...
        .alert("Reset Co-Sovereignty?", isPresented: $showingResetConfirmation) {
            ...
            Button("Reset Everything", role: .destructive) {
                Task {
                    await resetSovereignty()
                }
            }
        } message: {
            Text("This will clear all covenants, trust tiers, deadlock state, and comprehension status. You will need to complete the setup process again.")
        }
    }

    // MARK: - Reset Action

    private func resetSovereignty() async {
        isResetting = true
        await sovereigntyService.resetAll()
        isResetting = false
    }
