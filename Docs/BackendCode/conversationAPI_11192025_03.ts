...
import { ProviderRequest, ProviderResponse, ContentPart } from '../providers/types';
...
/**
 * Sanitize content for Firestore storage
 * Removes nested entities and complex objects that Firestore can't serialize
 */
function sanitizeContentForFirestore(content: string | ContentPart[]): string | any[] {
  if (typeof content === 'string') {
    return content;
  }

  // If it's an array (multipart content), sanitize each part
  return content.map((part: ContentPart) => {
    ...
    } else if (part.type === 'audio_base64') {
      return {
        type: 'audio_base64',
        media_type: part.media_type,
        data: part.data,
    ...
    } else if (part.type === 'video_base64') {
      return {
        type: 'video_base64',
        media_type: part.media_type,
        data: part.data,
    ...
    // Fallback for unknown types - just return the part as-is
    return part;
  });
}
...
export interface ConversationMessage {
  id: string;
  role: 'user' | 'assistant';
  content: string | ContentPart[]; // Update to support multipart content
  createdAt: number;
  updatedAt: number;
  metadata?: {
    model?: string;
  ...
  corsHandler(req, res, async () => {
    try {
      ...
      const sanitizedMessage = {
        ...message,
        content: sanitizeContentForFirestore(content),
      };
  ...
  corsHandler(req, res, async () => {
    try {
      ...
      if (includeMemories) {
        ...
        const conversationText = allMessages
          .filter(m => m.role === 'user')
          .map(m => typeof m.content === 'string' ? m.content : m.content.filter((p: ContentPart) => p.type === 'text').map((p: ContentPart) => (p as { type: 'text'; text: string }).text).join(' '))
          .join(' ');
...
IMPORTANT: Memory type must be EXACTLY "allocentric" or "egoic" (not fact, insight, etc.)`;
      ...
      const sanitizedAssistantMessage = {
        ...assistantMessage,
        content: sanitizeContentForFirestore(response.content as any),
      };
...
export const apiRegenerateMessage = functions.https.onRequest(async (req, res) => {
  ...
  corsHandler(req, res, async () => {
    ...
    try {
      ...
      // Sanitize content for Firestore
      const sanitizedNewAssistantMessage = {
        ...newAssistantMessage,
        content: sanitizeContentForFirestore(response.content as any),
      };
      ...
      if (replaceLastMessage) {
        ...
        await getDb()
          ...
          .set(sanitizedNewAssistantMessage);
...
export const apiOrchestrate = functions.https.onRequest(async (req, res) => {
  ...
  corsHandler(req, res, async () => {
    ...
    try {
      ...
      // Sanitize content for Firestore (handles nested entities in multipart content)
      const sanitizedUserMessage = {
        ...userMessage,
        content: sanitizeContentForFirestore(content),
      };
